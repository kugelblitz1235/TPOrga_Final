...
; Llenar el vector auxiliar con el valor SHRT_MIN
mov rsi, 0
mov rax, width
dec rax
.loop:
...
.loop1:
    ...
    ; Guardamos 0's para no afectar a los calculos
...
pmaxsw nums_xmm, nums_s_xmm                         ; nums_mm = | W W W W|	

movdqu nums_s_xmm, nums_xmm
psrldq nums_s_xmm, 2*2
pmaxsw nums_xmm, nums_s_xmm                         ; nums_mm = |   W   W|

movdqu nums_s_xmm, nums_xmm
psrldq nums_s_xmm, 4*2
pmaxsw nums_xmm, nums_s_xmm                         ; nums_mm = |       W|
...
bsf rax, rax                                                ; Obtener la posicion del word de valor máximo entre todos los de la diagonal
shr rax, vector_len_log

pextrw edx, nums_xmm, 0b0000
pextrd ecx, best_x_y_global, best_global_xmm_pos            ; Obtener el valor máximo

cmp ecx, edx                                                ; best_global < max_local_score
jge .menor_a_best

; Actualizar el máximo valor obtenido junto con su posición asociada
...
.no_debug:
; Recuperar los 2 strings del mejor alineamiento utilizando backtracking, empezando desde la posicion asociada al valor maximo obtenido
push rsi
push score_matrix
mov rsi, rdi
