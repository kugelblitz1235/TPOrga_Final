  ...
  // Directiva utilizada para emular la representación de un ymm con su parte xmm
	...
	void inicializar_casos_base(){
		...
		for(int i = 0 ; i < height ; i++){
			...
			_mm256_storeu_si256((__m256i*)(score_matrix + offset_y), zeroes_mm.y);
			_mm256_storeu_si256((__m256i*)(score_matrix + offset_y + vector_len), zeroes_mm.y);
		}
	}
	...
	void actualizar_posicion_maxima(int i, int j){
		...
		nums_s_mm.y = _mm256_srli_si256 (nums_mm.y,1*2);		
		nums_mm.y = _mm256_max_epi16 (nums_mm.y,nums_s_mm.y);					// nums_mm = | W W W W| W W W W|	
		nums_s_mm.y = _mm256_srli_si256  (nums_mm.y,2*2);
		nums_mm.y = _mm256_max_epi16 (nums_mm.y,nums_s_mm.y);					// nums_mm = |   W   W|   W   W|
		nums_s_mm.y = _mm256_srli_si256  (nums_mm.y,4*2);
		nums_mm.y = _mm256_max_epi16 (nums_mm.y,nums_s_mm.y);					// nums_mm = |       W|       W|

		nums_s_mm.y = _mm256_permute4x64_epi64(nums_mm.y, 0b00000010);
		nums_mm.y = _mm256_max_epi16 (nums_mm.y,nums_s_mm.y);					// nums_mm = |        |       W|

		__m128i max_ymm = _mm256_extracti128_si256(nums_mm.y, 0);
		nums_mm.y = _mm256_broadcastw_epi16(max_ymm);
		
		// Obtener el índice del valor máximo en el registro
		__m256i index_ymm = _mm256_cmpeq_epi16(nums_mm.y,diag_score_mm.y);
		...
		short max_local_score =  _mm256_extract_epi16 (nums_mm.y, 0b0000);	// Obtener el valor máximo
	...
	void SW(Alignment& alignment, bool debug){
		...
		for( int i = 0 ; i < height ; i++){
            ...
            for( int j = 2; j < width ; j++){
                ...
                diag_score_mm.y = _mm256_max_epi16(diag_score_mm.y,zeroes_mm.y);
